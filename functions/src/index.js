...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest(
  (request: {}, response: {}): void => {
    ...
    async function reponseTheAnswer(agent: {}) {
      ...
      const weatherQuestion: WeatherQuestionInfo = new WeatherQuestionInfo(
        body
      );
      ...
      if (!weatherQuestion.isAddressKnown()) {
        agent.add(
          "Dans quelle ville voullez-vous que je recherche cette information ?"
        );
      } else {
        try {
          ...
          const weatherInfo: WeatherQuestionInfo = new WetherApiSchema(
            reponseApi
          );

          // personalized the answer
          const finalResponseString: string = weatherResponse(
            weatherInfo,
            weatherQuestion
          );
          ...
          agent.add(
            "Une erreur est survenue lors de la récupération des infos météo."
          );
        }
      }
    }
